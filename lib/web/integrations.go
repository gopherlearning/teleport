	...
	for {
		...
		for _, dc := range discoveryConfigs {
			...
			if matchers := matchersWithIntegration(dc, types.AWSMatcherEC2, integration.GetName()); matchers != 0 {
				...
				if ret.AWSEC2.DiscoverLastSync.Before(dc.Status.LastSyncTime) {
					ret.AWSEC2.DiscoverLastSync = dc.Status.LastSyncTime
				}
			...
			if matchers := matchersWithIntegration(dc, types.AWSMatcherRDS, integration.GetName()); matchers != 0 {
				...
				if ret.AWSRDS.DiscoverLastSync.Before(dc.Status.LastSyncTime) {
					ret.AWSRDS.DiscoverLastSync = dc.Status.LastSyncTime
				}
			...
			if matchers := matchersWithIntegration(dc, types.AWSMatcherEKS, integration.GetName()); matchers != 0 {
				...
				if ret.AWSEKS.DiscoverLastSync.Before(dc.Status.LastSyncTime) {
					ret.AWSEKS.DiscoverLastSync = dc.Status.LastSyncTime
				}
